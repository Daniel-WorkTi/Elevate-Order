  ...
  function savePublicBase() {
    ...
    try {
      ...
      if (url.protocol !== "https:") {
        setBaseError(t("connections.httpsRequired"));
        return;
      }
      if (isLocalWebhookHost(url.origin)) {
        setBaseError(t("connections.noLocalhost"));
        return;
      }
      setPublicBase(url.origin);
      setBaseError(null);
    } catch {
      setBaseError(t("connections.invalidUrl"));
    }
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <p className="text-[14px] font-semibold text-[#0A0C10]">
          {t("connections.orderUpdateNotifications")}
        </p>
      </div>
      <p className="text-[13px] text-[#667085]">{t("connections.orderUpdateNotificationsHint")}</p>
      ...
      {isLocal ? (
        <div className="flex items-start gap-2 rounded-[10px] border border-amber-200 bg-amber-50 px-3 py-2.5 text-[12px] text-amber-900">
          ...
          <div>
            <p className="font-semibold">{t("connections.webhookNotReachable")}</p>
            <p className="mt-0.5">{t("connections.webhookNotReachableHint")}</p>
          </div>
        </div>
      ) : null}
      ...
      <label className="block space-y-1.5">
        <span className="text-[12px] font-medium text-[#667085]">
          {t("connections.publicDeployBase")}
        </span>
      ...
      <label className="block space-y-1.5">
        <span className="text-[12px] font-medium text-[#667085]">{t("connections.webhookUrl")}</span>
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          <Input
            readOnly
            value={webhookUrl}
