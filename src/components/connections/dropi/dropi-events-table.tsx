  ...
  return (
    <section className="rounded-[16px] border border-[#E6E8EC] bg-white p-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="text-[15px] font-semibold text-[#0A0C10]">Recent webhook events</h2>
          <p className="mt-1 text-[13px] text-[#667085]">
            Persisted Dropi events from <code className="text-[11px]">order_events</code>.
          </p>
        </div>
        {showFilters ? (
      ...
      {filtered.length === 0 ? (
        <div className="mt-6 rounded-[12px] border border-dashed border-[#E6E8EC] px-4 py-10 text-center">
          <p className="text-[14px] font-medium text-[#0A0C10]">No Dropi events received yet.</p>
          <p className="mt-1 text-[13px] text-[#667085]">
            {emptyHint ??
                ...
                {filtered.map((event) => (
                  ...
                  >
                    <td className="px-3 py-3 tabular-nums text-[#667085]">
                      {formatDropiTime(event.eventDate)}
                    </td>
            ...
            {filtered.map((event) => (
              ...
              >
                ...
                <p className="mt-1 text-[12px] text-[#667085]">
                  {event.statusName ?? "order.updated"} · Processed
                </p>
              </button>
            ))}
          </div>
