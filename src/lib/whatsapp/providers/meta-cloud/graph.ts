import type { WhatsAppServerConfig } from "@/lib/whatsapp/providers/meta-cloud/config";
import { sanitizeMetaErrorForLog } from "@/lib/whatsapp/errors";
...
export async function exchangeEmbeddedSignupCode(
  ...
  const redirectCandidates = [null, redirectUriOverride?.trim() || null, config.redirectUri].filter(
  ...
  for (let index = 0; index < redirectCandidates.length; index += 1) {
    const redirectUri = redirectCandidates[index]!;
    try {
      return await exchangeEmbeddedSignupCodeOnce(config, code, fetchImpl, redirectUri);
    } catch (error) {
      lastError = error instanceof Error ? error : new Error(String(error));
      const isLast = index === redirectCandidates.length - 1;
      if (isLast || isNonRetryableOAuthExchangeError(lastError)) {
        throw lastError;
      }
    }
  }

  throw lastError ?? new Error("Meta token exchange failed.");
}

function isNonRetryableOAuthExchangeError(error: Error): boolean {
...
async function exchangeEmbeddedSignupCodeOnce(
  config: WhatsAppServerConfig,
  code: string,
...
export async function resolveAuthorizedWhatsAppResources(
  ...
  const wabaId =
    sessionHints.wabaId && debug.wabaIds.includes(sessionHints.wabaId)
      ? sessionHints.wabaId
      : (debug.wabaIds[0] ?? sessionHints.wabaId ?? null);
  ...
  let phoneNumberId: string | null =
    sessionHints.phoneNumberId && debug.phoneNumberIds.includes(sessionHints.phoneNumberId)
      ? sessionHints.phoneNumberId
      : (debug.phoneNumberIds[0] ?? sessionHints.phoneNumberId ?? null);
